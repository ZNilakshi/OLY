...
const ListingDetails = () => {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        const response = await fetch("http://oly-production-ce41.up.railway.app/api/auth/user", {
          credentials: "include",
        });
  ...
  useEffect(() => {
    const fetchListing = async () => {
      try {
        const response = await fetch(`http://oly-production-ce41.up.railway.app/api/listings/${id}`);
        ...
        <div className="space-y-4">
          <div className="relative">
            <img
              ...
              alt="Listing Preview"
              className="w-full h-97 object-cover rounded-sm shadow-md"
            />
          </div>
        ...
        <div className="space-y-6">
          <div className="bg-gray-50 p-6 rounded-sm shadow-sm">
            ...
            <p className="text-2xl font-semibold text-gray-900">
              {listing.price === 0 || listing.price === null ? "Free" : `Rs.${listing.price}.00`}
            </p>
          </div>
              ...
              <div className="flex items-center space-x-4">
                <Link to={`/user/${listing.userId._id}`}>
                  <img
                    ...
                    alt="User Profile"
                    className="w-16 h-17 rounded-full cursor-pointer hover:opacity-75 transition-opacity"
                  />
                </Link>
                <div>
