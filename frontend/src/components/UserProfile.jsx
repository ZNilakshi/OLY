...
const UserProfile = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch current user
        const currentUserRes = await fetch("http://oly-production-ce41.up.railway.app/user");
        ...
        // Fetch user details
        const userResponse = await fetch(`http://oly-production-ce41.up.railway.app/api/users/${userId}`);
        if (!userResponse.ok) throw new Error("Failed to fetch user data");
        const userData = await userResponse.json();
        setUser(userData);

        // Fetch user listings
        const listingsResponse = await fetch(`http://oly-production-ce41.up.railway.app/api/listings/user/${userId}`);
